...
export default class VideoRecorder extends Component {
  ...
  static defaultProps = {
    locales: {},
    renderUnsupportedView: (props) => (
      <UnsupportedView locales={props ? props : {}} />
    ),
    renderErrorView: (props) => <ErrorView locales={props ? props : {}} />,
    renderVideoInputView: ({ videoInput }) => <>{videoInput}</>,
    renderDisconnectedView: () => <DisconnectedView />,
    renderLoadingView: (props) => <LoadingView locales={props ? props : {}} />,
    renderActions,
    isFlipped: true,
    countdownTime: 3000,
  ...
  renderCameraView() {
    ...
    if (!isInlineRecordingSupported) {
      return renderUnsupportedView(locales);
    }
